...
//////////////////// SET INITIAL VARIABLES AND SETUP ARRAY/LOCAL STORAGE DATABASE ////////////////////
  ...
  let recordsArray = [];
  // Using self initiating ternary operator to retrieve data if it exists or keep empty array otherwise.
...
//////////////////// FUNCTIONS TO BE CALLED IN THE CODE CREATED AS THE CODE IS REPEATED HENCE FUNCTIONS ARE EFFICIENT ////////////////////
  ...
  let quizUpdate = (answerCopy) => {
    ...
    if (qCount === questions.length) {
        queryElement('#quizHolder').classList.add('hide');
        queryElement('#finish').classList.remove('hide');
        time = 0;
...
queryElement("#intro button").addEventListener("click", (e) => {
    queryElement('#intro').classList.add('hide');
    queryElement('#quizHolder').classList.remove('hide');
...
// Displays error message if initials given do not meet requirments
  let errorIndicator = () => {
    clearTimeout(timeset);
    timeset = setTimeout(() => {
...
queryElement("#records button").addEventListener("click", () => {
      ...
      if (initialsRecord === '') {
          ...
          queryElement('#highScores div').innerHTML = '';
          queryElement('#finish').classList.add('hide');
          queryElement('#highScores').classList.remove('hide');
          recordsHtmlReset();
...
//////////////////// HIGH SCORE CONTROL ARRAY/LOCAL STORAGE ////////////////////
  ...
  queryElement("#reset").addEventListener("click", () => {
      ...
      queryElement('#intro').classList.remove('hide');
  });
